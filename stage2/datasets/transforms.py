...
class CenterPad_vec(object):
    ...
    def __call__(self, image):
        ...
        if height_old / width_old > ratio:  # pad the width and crop
            ...
            resize_ratio = height / height_old
        #return Image.fromarray(I_pad.astype(np.uint8))
        return I_pad * resize_ratio

class CenterPad_mask():
    def __init__(self, image_size):
        self.height = image_size[0]
        self.width = image_size[1]
    ...
    def __call__(self, image):
        # pad the image to 16:9
        # pad height
        I = np.array(image)                 # h w 256
        I = np.sum(I, axis = 2,keepdims=True)
        #print("debug0:",I.max(),Imean,I.min(),I.shape)
        I [I<10]=0
        I [I>10]=255
        I = np.tile(I,(1,1,3))
        #print("debug:",I.max(),I.mean(),I.shape)
        # for padded input                  要求输入是 H W C
        height_old = np.size(I, 0)
        width_old = np.size(I, 1)
        old_size = [height_old, width_old]
        ...
        if height_old / width_old > ratio:  # pad the width and crop
           ...
           # print("shape: ",I_resize.shape)
            ...
            start_width = (width_resize - width) // 2
            I_pad[:, :, :] = I_resize[:, start_width : (start_width + width), :]
        #print("debug:",I_pad.shape,I_pad.dtype)
        return Image.fromarray(I_pad.astype(np.uint8))
...
def to_mytensor(pic):
    ...
    img = torch.from_numpy(pic_arr.transpose((2, 0, 1)))          #HWC  to  C H W
...
class Normalize(object):
    ...
    def __call__(self, inputs):
        inputs[0:1, :, :] = F.normalize(inputs[0:1, :, :], 50, 1)                # mean std
