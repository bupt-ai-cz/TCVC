...
class MultiheadAttention_VC(Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=True, add_zero_attn=True, kdim=None,
                 vdim=None):
        super(MultiheadAttention_VC, self).__init__()
    ...
    def forward(self, query, key, value, key_padding_mask=None, need_weights=True, attn_mask=None,temperature=1):
        r"""
    Args:
    ...
    Shape:
        ...
        if hasattr(self, '_qkv_same_embed_dim') and self._qkv_same_embed_dim is False:
            return multi_head_attention_forward(
                ...
                attn_mask=attn_mask, use_separate_proj_weight=True,                      #--------different from here----------#
                q_proj_weight=self.q_proj_weight, k_proj_weight=self.k_proj_weight,
                v_proj_weight=self.v_proj_weight,temperature=temperature)
        else:
            ...
            return multi_head_attention_forward(
                ...
                self.bias_k, self.bias_v, self.add_zero_attn,
                self.dropout, self.out_proj.weight, self.out_proj.bias,
                training=self.training,                                                     #-----在哪？
                key_padding_mask=key_padding_mask, need_weights=need_weights,
                attn_mask=attn_mask,temperature=temperature)
...
def multi_head_attention_forward(query,  # type: Tensor
    ...
    assert key.size() == value.size()                                                  #----key value的shape要相等
    ...
    #q = q - q.max(dim=1, keepdim=True)[0]                      # b*head  nhw  c
    ...
    if attn_mask is not None:
        # for map in attn_output_weights:                      # B * head , n_q ,n_k
        #     map[attn_mask==0]=-10                 
        attn_output_weights -= attn_mask.unsqueeze(0) * 10
        # attn_output_weights = attn_output_weights.masked_fill(
        #     attn_mask.unsqueeze(0),                 # b nhw
        #     float('-inf'),
        # )
    ...
    if key_padding_mask is not None:
        ...
        attn_output_weights = attn_output_weights.masked_fill(
            key_padding_mask.unsqueeze(1).unsqueeze(2),                 # b nhw
            float('-inf'),
        )
        attn_output_weights = attn_output_weights.view(bsz * num_heads, tgt_len, src_len)

    # AttentionScore = Softmax(AttentionWeights)
    attn_output_weights = attn_output_weights / temperature
    attn_output_weights = attn_output_weights#.float()
    attn_output_weights = F.softmax(attn_output_weights, dim=-1)#.half()
    ...
    #-------------所以实际上是将维度拆分来做多个attention，然后再拼到一起。attentionmap对head取平均。