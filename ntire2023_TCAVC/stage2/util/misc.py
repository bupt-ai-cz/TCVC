...
def collate_fn(batch):
    #print("collete_fn:",len(batch))                               # 1
    batch = list(zip(*batch))                                     # *batch = batch[0] , batch[1]...
    #print("coll 2:",len(batch))                                   # 2
    batch[0] = nested_tensor_from_tensor_list(batch[0])
    return tuple(batch)                                           #将列表转换为元组
...
def nested_tensor_from_tensor_list(tensor_list: List[Tensor], split=True):
    ...
    #print("nested",len(tensor_list),tensor_list[0].shape)     #1 , [36, 3, 256, 256]
    if split:
    ...
    if tensor_list[0].ndim == 3:
        ...
        #print("nested",tensor_list[0].shape)       # torch.Size([3, 3, 256, 256])
