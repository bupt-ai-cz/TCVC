...
def load_images(path,gray=False):
    ...
    #print("debug:",len(image_paths),H,W)
    image_paths.sort()
...
def get_fid_video(path1,path2,batch_size,use_multiprocessing = True):
    fid_values=[]
    subdirs = sorted(os.listdir(path2))
    for subdir in subdirs:
        _path1 = os.path.join(path1,subdir)
        _path2 = os.path.join(path2,subdir)
        fid = get_fid(_path1,_path2,batch_size=batch_size,use_multiprocessing=use_multiprocessing)
...
if __name__ == "__main__":
    ...
    parser.add_option("--p2", "--path2", dest="path2", default = '/dataset/checkpoints/spcolor/val_10000/stego/1',
    ...
    images2 = load_images(options.path2,gray=False)
